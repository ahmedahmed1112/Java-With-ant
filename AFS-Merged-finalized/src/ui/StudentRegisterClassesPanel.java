...
public class StudentRegisterClassesPanel extends JPanel {
    ...
    private void loadData() {
        ...
        Set<String> registeredClassIds = new HashSet<>();
        if (!stuId.isEmpty()) {
            List<String[]> registered = StudentService.getRegisteredClasses(stuId);
            for (String[] r : registered) {
                registeredModel.addRow(r);
                if (r.length >= 2) registeredClassIds.add(r[1].toUpperCase());
            }
        }

        // Available classes
        availableModel.setRowCount(0);
        // Many-to-many registration: show all classes, then hide already registered ones.
        List<String[]> classes = StudentService.getAvailableClasses(null);
        for (String[] c : classes) {
            if (c.length >= 1 && registeredClassIds.contains(c[0].toUpperCase())) continue;
